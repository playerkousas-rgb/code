...
function updateAll() {
    ...
    $('outMorse').textContent = upper.split('').map(c => MORSE_CODE[c] || c).join(' ');
    ...
    $('outBraille').innerHTML = upper.split('').map(c => c===' '?'<span class="w-8"></span>':renderBrailleChar(c)).join('');
    $('outPigpen').innerHTML = upper.split('').map(c => c===' '?'<span class="mx-4 text-amber-500 font-bold">/</span>':renderPigpenSVG(c)).join('');
    
    const key = ($('gridKey').value || 'SCOUT').toUpperCase().replace(/[^A-Z]/g,'').slice(0,5);
    $('outGrid').textContent = encodeGrid(upper, key);

    const shift = parseInt($('caesarShift').value) || 3;
    const s = ($('caesarDir').value === 'forward') ? shift : (26 - shift);
    $('outCaesar').textContent = upper.replace(/[A-Z]/g, c => String.fromCharCode(65 + (c.charCodeAt(0) - 65 + s) % 26));
    $('outAtbash').textContent = upper.replace(/[A-Z]/g, c => String.fromCharCode(90 - (c.charCodeAt(0) - 65)));
    
    $('outCangjie').textContent = upper.split('').map(c => c===' '?'/':(CANGJIE_MAP[c] || c)).join(' ');
    $('outPinyin').textContent = upper.split('').map(c => c===' '?'/':(PINYIN_MAP[c] || c)).join(' ');
    $('outJyutping').textContent = upper.split('').map(c => c===' '?'/':(JYUTPING_MAP[c] || c)).join(' ');
...
function renderPigpenSVG(c) {
    ...
    return `<svg class="pigpen-svg" width="40" height="40" viewBox="0 0 45 45"><path d="${path}" fill="none" stroke="#ffcc00" stroke-width="3" stroke-linecap="round"/><circle cx="${dotPos.x}" cy="${dotPos.y}" r="${dot?3:0}" fill="#ffcc00"/></svg>`;
}
...
function buildReferenceTables() {
    ...
    $('cangjieTableWrap').innerHTML = h + '</div>';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $('btnLockGame').onclick = () => { gameMode='locked'; broadcast({type:'modeUpdate',mode:'locked'}); };
    $('btnJoinConfirm').onclick = () => {
        const name=$('playerName').value.trim(); if(!name) return;
        hostConn.send({type:'join',name}); $('memberJoinForm').style.display='none'; $('memberBuzzer').style.display='flex'; $('displayMyName').textContent=name;
    };
    $('btnBuzzer').onclick = handleBuzz;

    const urlParams = new URLSearchParams(window.location.search);
    if(urlParams.get('join')) { $('mainContent').style.display='none'; $('sidebar').style.display='none'; $('memberView').style.display='flex'; initMember(urlParams.get('join')); }
...
function showProjection() {
    ...
    if(q.type==='Morse') html = `<div class="text-[7vw] font-mono tracking-widest text-[var(--skw-gold)]">${upper.split('').map(c=>MORSE_CODE[c]||c).join(' ')}</div>`;
    ...
    $('projectionContent').innerHTML = `<div class="text-center">${html}<div class="mt-20 text-slate-700 font-bold text-2xl tracking-[1em]">SKWSCOUT</div></div>`;
}
...
window.onbeforeprint = () => {
    $('printQuestions').innerHTML = testQuestions.map((q, idx) => {
        let encoded = '';
        const upper = q.text.toUpperCase();
        if(q.type === 'Morse') encoded = `<span class="text-2xl font-mono">${upper.split('').map(c => MORSE_CODE[c]||c).join(' ')}</span>`;
        ...
        return `<div class="mb-12 break-inside-avoid"><p class="text-lg font-bold mb-4">Q${idx+1}. 翻譯以下密碼 (${q.type}):</p><div class="p-8 border-2 border-black rounded flex items-center justify-center min-h-[120px]">${encoded}</div><div class="mt-4 border-b border-black h-8"></div></div>`;
    }).join('');
};
...
function updateHostUI() {
    ...
    $('buzzList').innerHTML = buzzQueue.map((p,idx)=>`<div class="skw-card p-4 bg-[var(--skw-gold)] text-black text-center font-bold"><span class="text-2xl block">#${idx+1}</span>${p.name}</div>`).join('');
}
function initMember(hostId) {
    ...
    peer.on('open', () => {
        ...
        hostConn.on('data', (data) => {
            if(data.type==='modeUpdate') { gameMode=data.mode; $('btnBuzzer').classList.toggle('disabled', gameMode!=='open'); $('buzzStatus').textContent = gameMode==='open'?'開放搶答中！':'請等待領袖...'; }
            if(data.type==='buzzUpdate') { if(data.queue.find(p=>p.id===peer.id)) { $('btnBuzzer').classList.add('disabled'); $('buzzStatus').textContent='已搶答！'; if(navigator.vibrate) navigator.vibrate(200); } }
        });
    });
}
function handleBuzz() { if(gameMode==='open' && hostConn) hostConn.send({type:'buzz'}); }
function renderTestQuestionList() {
    $('testQuestionList').innerHTML = testQuestions.map((q,idx)=>`
        <div class="skw-card p-4 flex justify-between items-center mb-0">
            <div class="flex items-center gap-4"><span class="font-black text-[var(--skw-gold)]">Q${idx+1}</span><span class="text-white">${q.text}</span>
            <select onchange="testQuestions[${idx}].type=this.value;localStorage.setItem('skw_test_questions',JSON.stringify(testQuestions))" class="bg-black/40 text-xs border border-white/10 rounded px-2 py-1">
                <option value="Morse" ${q.type==='Morse'?'selected':''}>摩斯</option><option value="Semaphore" ${q.type==='Semaphore'?'selected':''}>旗號</option><option value="Braille" ${q.type==='Braille'?'selected':''}>點字</option><option value="Text" ${q.type==='Text'?'selected':''}>原文</option>
            </select></div>
            <button onclick="testQuestions.splice(${idx},1);localStorage.setItem('skw_test_questions',JSON.stringify(testQuestions));renderTestQuestionList();" class="text-red-400 text-xs">刪除</button>
        </div>`).join('');
}
function playMorse() {
